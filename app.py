...
.metric-card { background: linear-gradient(135deg, #1e293b, #0f172a); border: 1px solid #334155; border-radius: 16px; padding: 1.25rem 1.5rem; position: relative; overflow: hidden; transition: transform 0.2s, box-shadow 0.2s; margin-bottom: 0.5rem; }
...
.feature-card { background: #1e293b; border: 1px solid #334155; border-radius: 12px; padding: 1.2rem; margin-bottom: 0.75rem; transition: border-color 0.2s; }
...
.task-pill { display: inline-flex; align-items: center; padding: 0.25rem 0.7rem; border-radius: 999px; font-size: 0.75rem; font-weight: 600; }
...
.stButton > button { background: linear-gradient(135deg, #6366f1, #8b5cf6) !important; color: white !important; border: none !important; border-radius: 10px !important; padding: 0.55rem 1.4rem !important; font-weight: 600 !important; font-size: 0.88rem !important; transition: all 0.2s !important; box-shadow: 0 4px 14px rgba(99,102,241,0.35) !important; }
.stButton > button:hover { transform: translateY(-1px) !important; box-shadow: 0 6px 20px rgba(99,102,241,0.5) !important; }
.stTextInput input, .stTextArea textarea { background: #1e293b !important; border: 1px solid #334155 !important; border-radius: 10px !important; color: #f1f5f9 !important; }
...
# AUTO LOAD API KEY - tries st.secrets first, then os.environ
def get_api_key():
    try:
...
with st.sidebar:
    ...
    """, unsafe_allow_html=True)
    st.markdown("---")
# AUTO CONNECT STATUS (No input field shown)
if st.session_state.api_key_valid:
    st.markdown("""
    ...
    border-radius:10px; padding:0.6rem 1rem; color:#f87171; font-size:0.82rem; font-weight:600;'>
        ⚠️ AI Not Connected
    </div>
    """, unsafe_allow_html=True)
...
if current_page == "Dashboard":
    ...
    st.markdown(f"<div style='margin-bottom:1.5rem;'><div style='font-size:1.7rem; font-weight:800; color:#f1f5f9;'>Good {greet}</div><div style='color:#64748b; font-size:0.9rem; margin-top:0.2rem;'>{datetime.now().strftime('%A, %B %d, %Y')} — Your AI workspace is ready.</div></div>", unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    for col, color, num, label, delta, up in [(c1,"purple","47","Tasks Completed","↑ 12% this week",True),(c2,"teal","8","Emails Drafted","↑ 3 today",True),(c3,"green","5","Meetings Logged","↓ 1 from last wk",False),(c4,"amber","3","Reports Ready","↑ 2 new",True)]:
        with col:
            delta_cls = "delta-up" if up else "delta-down"
            st.markdown(f"<div class='metric-card {color}'><div class='metric-number'>{num}</div><div class='metric-label'>{label}</div><div class='metric-delta {delta_cls}'>{delta}</div></div>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    col_l, col_r = st.columns([3, 2])
    with col_l:
        st.markdown("<div class='section-header'>📈 Weekly Productivity Gains</div>", unsafe_allow_html=True)
        df = pd.DataFrame({"Department": ["Sales","HR","Support","Admin","Marketing","Engineering"], "Hours Saved": [10,8,15,6,11,13], "Tasks Automated": [22,15,30,10,20,28]})
        fig = go.Figure()
        ...
        fig.update_layout(barmode="group", height=280, plot_bgcolor="#0f172a", paper_bgcolor="#1e293b", font=dict(color="#94a3b8", size=12, family="Inter"), legend=dict(bgcolor="rgba(0,0,0,0)"), margin=dict(l=10,r=10,t=10,b=10), xaxis=dict(gridcolor="#334155"), yaxis=dict(gridcolor="#334155"))
        st.plotly_chart(fig, use_container_width=True)
    with col_r:
        st.markdown("<div class='section-header'>🔥 Feature Usage</div>", unsafe_allow_html=True)
        pie_df = pd.DataFrame({"Feature": ["Email Writer","Task Manager","Summarizer","Reports","Customer Reply"], "Usage": [32,27,20,12,9]})
        fig2 = px.pie(pie_df, names="Feature", values="Usage", hole=0.45, color_discrete_sequence=["#6366f1","#06b6d4","#10b981","#f59e0b","#f43f5e"])
        fig2.update_layout(height=280, plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="#1e293b", font=dict(color="#94a3b8", family="Inter"), legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(size=11)), margin=dict(l=10,r=10,t=10,b=10))
        fig2.update_traces(textfont_color="white")
        st.plotly_chart(fig2, use_container_width=True)
    st.markdown("<div class='section-header'>⚡ Quick Access</div>", unsafe_allow_html=True)
    qa1, qa2, qa3, qa4 = st.columns(4)
    for col, icon, title, desc in [(qa1,"✉️","Draft Email","AI-powered in seconds"),(qa2,"✅","Add Task","Smart prioritization"),(qa3,"🎙️","Summarize Meeting","Notes → Action items"),(qa4,"📈","Generate Report","Weekly executive summary")]:
        with col:
            st.markdown(f"<div class='feature-card'><div style='font-size:1.4rem;'>{icon}</div><div style='font-weight:600; color:#f1f5f9; margin-top:0.3rem;'>{title}</div><div style='color:#64748b; font-size:0.8rem;'>{desc}</div></div>", unsafe_allow_html=True)
        ...
        if st.button("🚀 Summarize Meeting"):
            if not notes.strip():
                ...
                with st.spinner("Analyzing meeting..."):
                    result = ask_ai(f"Analyze this meeting and provide:\n1. Executive Summary\n2. Key Discussion Points\n3. Action Items\n4. Decisions Made\n5. Next Steps\nFormat: {output_fmt}. Language: {lang}.\nMeeting notes:\n{notes}")
                st.markdown(f"<div class='ai-output'>{result}</div>", unsafe_allow_html=True)
                st.session_state.meetings.append({"preview": notes[:60]+"...", "summary": result, "time": datetime.now().strftime("%b %d, %H:%M")})
    with tab2:
    ...
    with ca:
        if st.button("➕ Add Task"):
            if task_input:
                st.session_state.tasks.append({"task": task_input, "priority": priority, "category": category, "done": False, "added": datetime.now().strftime("%H:%M"), "ai_suggestion": ""})
                st.rerun()
    with cb:
        if st.button("🧠 AI Prioritize All"):
            if st.session_state.tasks:
                ...
                with st.spinner("AI analyzing..."):
                    result = ask_ai(f"Reorder by urgency + impact, add a 1-line tip for each:\n{tlist}", "You are a productivity expert.")
                st.markdown(f"<div class='ai-output'>{result}</div>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    if not st.session_state.tasks:
        ...
        for i, task in enumerate(st.session_state.tasks):
            ...
            with col_task:
                ...
                st.markdown(f"<div style='padding:0.5rem 0;'><span style='{style} font-weight:500;'>{task['task']}</span><span class='task-pill {pri_cls}' style='margin-left:0.5rem;'>{task['priority']}</span><span style='color:#475569; font-size:0.76rem; margin-left:0.5rem;'>{task['category']} · {task['added']}</span></div>", unsafe_allow_html=True)
            with col_tip:
                if st.button("💡", key=f"tip_{i}"):
                    with st.spinner("..."):
                        tip = ask_ai(f"1-sentence smart tip to complete: '{task['task']}'")
            ...
            if task.get("ai_suggestion"):
                st.markdown(f"<div style='margin:0.1rem 0 0.5rem 2rem; padding:0.4rem 0.7rem; background:#0f172a; border-left:3px solid #6366f1; border-radius:0 8px 8px 0; color:#a5b4fc; font-size:0.82rem;'>💡 {task['ai_suggestion']}</div>", unsafe_allow_html=True)
    ...
    if st.button("🚀 Generate Reply"):
        if not customer_msg.strip():
            ...
            with st.spinner("Crafting reply..."):
                result = ask_ai(f"Customer support for {industry}. Mood: {sentiment}. Style: {reply_tone}. Context: {context or 'Not specified'}. Message: \"{customer_msg}\". Write professional empathetic reply.")
            st.markdown(f"<div class='ai-output'>{result}</div>", unsafe_allow_html=True)
            with st.spinner("Analyzing..."):
                analysis = ask_ai(f"2 bullet points: (1) Sentiment (2) Key concern: \"{customer_msg}\"")
    ...
    if st.button("📊 Generate Report"):
        with st.spinner("Generating report..."):
            result = ask_ai(f"Professional weekly report:\nTeam: {team_name or 'Team'} | Period: {week or 'This week'}\nAccomplishments: {accomplishments}\nBlockers: {blockers}\nNext Week: {next_week}\nKPIs: {kpis}\nFormat: Executive Summary, Achievements, Challenges, KPI Summary, Next Week Priorities, Overall Status (Green/Yellow/Red).")
        st.markdown(f"<div class='ai-output'>{result}</div>", unsafe_allow_html=True)
        st.session_state.reports.append({"team": team_name or "Team", "week": week or "This week", "content": result, "time": datetime.now().strftime("%b %d")})
